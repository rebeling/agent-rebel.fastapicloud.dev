...
def lint_all(conn: sqlite3.Connection) -> list[dict[str, Any]]:
    ...
    linked_targets = {
        ...
        for row in conn.execute(
            "SELECT target_page_id FROM links WHERE target_page_id IS NOT NULL"
        )
    }
    for page in pages:
    ...
    if page["page_type"] not in PAGE_TYPES:
        results.append(
            error(
                slug,
                "invalid-type",
                f"{page['page_type']} is not an allowed page type.",
            )
        )
    try:
        ...
        results.append(
            error(slug, "invalid-tags", "tags must be stored as a JSON list.")
        )
        tags = []
...
def error(
    slug: str, code: str, message: str, metadata: dict[str, Any] | None = None
) -> dict[str, Any]:
